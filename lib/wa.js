...
const { getRandom3  } = require("./help");
const fs = require("fs");

const erdwpe = connect.erdwpe;
const bufferFakeReply = fs.readFileSync("./lib/image/erdwpe.png");
...
exports.FakeStatusImgForwarded = (from, image, caption, faketeks) => {
  erdwpe.sendMessage(from, image, MessageType.image, {
    quoted: {
      key: {
        fromMe: false,
        participant: `0@s.whatsapp.net`,
...
exports.sendMessage = async (from, text) => {
  await erdwpe.sendMessage(from, text, MessageType.text);
};

exports.sendAudio = async (from, buffer) => {
  await erdwpe.sendMessage(from, buffer, MessageType.mp4Audio, {
    mimetype: Mimetype.mp4Audio,
    ptt: true,
  });
};

exports.sendImage = async (from, buffer, caption = "succses") => {
  await erdwpe.sendMessage(from, buffer, MessageType.image, {
    caption: caption,
  });
};

exports.sendVideo = async (from, buffer, caption = "") => {
  await erdwpe.sendMessage(from, buffer, MessageType.video, {
    caption: caption,
  });
};

exports.sendSticker = async (from, buffer) => {
  await erdwpe.sendMessage(from, buffer, MessageType.sticker);
};

exports.sendPdf = async (from, buffer, title = "LoL Human.pdf") => {
  await erdwpe.sendMessage(from, buffer, MessageType.document, {
...
exports.sendImageMention = async (from, buffer, text, mentioned) => {
  await erdwpe.sendMessage(from, buffer, MessageType.image, {
    contextInfo: { mentionedJid: [mentioned], participant: [mentioned] },
    caption: text,
  });
};
...
exports.sendFakeStatus = async (from, text, faketext, mentioned = []) => {
  ...
  await erdwpe.sendMessage(from, text, MessageType.text, options);
};
...
exports.sendFakeThumb = async (from, buffer, caption = "") => {
  ...
  await erdwpe.sendMessage(from, buffer, MessageType.image, options);
};

exports.downloadMedia = async (media) => {
  const filePath = await erdwpe.downloadAndSaveMediaMessage(
    media,
    `./temp/${getRandom3()}`
  );
...
exports.blockUser = async (id, block) => {
  if (block) return await erdwpe.blockUser(id, "add");
  await erdwpe.blockUser(id, "remove");
};

exports.getGroupParticipants = async (id) => {
  var members = await erdwpe.groupMetadata(id);
...
exports.getGroupInvitationCode = async (id) => {
  const linkgc = await erdwpe.groupInviteCode(id);
...
exports.kick = function (from, orangnya) {
  for (let i of orangnya) {
    erdwpe.groupRemove(from, [i]);
  }
};

exports.add = function (from, orangnya) {
  erdwpe.groupAdd(from, orangnya);
};

exports.kickMember = async (id, target = []) => {
  const group = await erdwpe.groupMetadata(id);
  const owner = g.owner.replace("c.us", "s.whatsapp.net");
  const me = erdwpe.user.jid;
  for (i of target) {
    if (!i.includes(me) && !i.includes(owner)) {
      await erdwpe.groupRemove(to, [i]);
    } else {
...
exports.promoteAdmin = async (to, target = []) => {
  const g = await erdwpe.groupMetadata(to);
  const owner = g.owner.replace("c.us", "s.whatsapp.net");
  const me = erdwpe.user.jid;
  for (i of target) {
    if (!i.includes(me) && !i.includes(owner)) {
      await erdwpe.groupMakeAdmin(to, [i]);
    } else {
...
exports.demoteAdmin = async (to, target = []) => {
  ...
  for (i of target) {
    if (!i.includes(me) && !i.includes(owner)) {
      await erdwpe.groupDemoteAdmin(to, [i]);
    } else {
      await this.sendMessage(to, "Not Premited!");
      break;
    }
...
exports.getUserName = async (jid) => {
  const user = erdwpe.contacts[jid];
  return user != undefined ? user.notify : "";
};

exports.getBio = async (mids) => {
  const pdata = await erdwpe.getStatus(mids);
  if (pdata.status == 401) {
    return pdata.status;
  } else {
    return pdata.status;
...
exports.getPictProfile = async (mids) => {
  try {
    var url = await erdwpe.getProfilePicture(mids);
  } catch {
    var url =
