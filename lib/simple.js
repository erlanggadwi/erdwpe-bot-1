...
exports.WAConnection = (_WAConnection) => {
  class WAConnection extends _WAConnection {
    constructor(...args) {
      ...
      this._events["CB:action,add:relay,message"].unshift(async function (
        ...
        try {
          ...
          if (
            lin.message &&
            lin.message.protocolMessage &&
            lin.message.protocolMessage.type == 0
          ) {
            let key = lin.message.protocolMessage.key;
    ...
    async sendSticker(jid, media, lin, opts = {}) {
      ...
      return this.sendMessage(
        ...
        await convert(
          medias.data,
          null,
          opts.packname || "Erdwpe Bot",
          opts.author || "erdwpe.xyz"
        ),
        "stickerMessage",
        { quoted: lin, ...opts }
      );
    ...
    async genOrderMessage(message, options) {
      ...
      switch (type) {
        ...
        case MessageType.audio:
          ...
          break;
        case "orderMessage":
          lin.orderMessage = WAMessageProto.OrderMessage.fromObject(message);
      }
...
`.trim();
    ...
    async prepareMessageMedia(buffer, mediaType, options = {}) {
      ...
      const { encBodyPath, bodyPath, fileLength, didSaveToTmpPath } =
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      if (lin.quoted.url)
        lin.quoted.download = (type = "buffer") => conn.downloadM(vM, type);
      /**
