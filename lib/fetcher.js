...
const aaas = require("./help");
const getRandoam = aaas.getRandom3;
...
exports.modStick = (media, erdwpe, lin, from) => {
  out = getRandoam(".webp");
  try {
    console.log(media);
    spawn("webpmux", ["-set", "exif", "./stik/data.exif", media, "-o", out]).on(
      "exit",
      () => {
        erdwpe.sendMessage(from, fs.readFileSync(out), "stickerMessage", {
          quoted: lin,
        });
        fs.unlinkSync(out);
        fs.unlinkSync(media);
    ...
    console.log(e);
    erdwpe.sendMessage(from, "Terjadi keslahan", "conversation", {
      quoted: lin,
    });
    fs.unlinkSync(media);
  }
